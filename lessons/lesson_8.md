...
## Теоретическая часть
 ...
 - миграции  
 - транзакции  
 - индексы - какие бывают, как строить  
 - добавление колонок  
 - профилирование запросов  

# TODO
нужны запросы с join, group by, order  
нужна большая коллекция, поиск по которой без индексов будет долго работать  
нужна задача, которая бы требовала наличия транзакций  
нужно описание того, как добавлять колонки на горячую (не факт)  
...
### Задание 1
Описать все сущности, хранящиеся в БД, как схемы Sequelize и методы схем. Каждая сущность должна быть описана в отдельном файле. Например пользователь - `model/user.js`. Файл должен экспортировать уже готовую модель. Для удобства можно подключить все модели куда-нибудь в `global.model = { ... }`



### Задание 2
Переписать все запросы к БД на Sequelize
